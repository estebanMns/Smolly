...
class _LevelmapState extends State<Levelmap> with TickerProviderStateMixin {
  ...
  void _initializeLevels(int maxUnlocked) {
    ...
    levels = List.generate(22, (index) {
      ...
      return LevelData(
        ...
        name: index == 0
            ? 'tutorial'.tr
            : (index == 21 ? 'Nivel extra 21' : '${'level'.tr} $index'),
        status: status,
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      body: Obx(() {
        final maxUnlocked =
            profileController.player.value?.maxUnlockedLevel ?? 1;
        _initializeLevels(maxUnlocked);
    ...
    return Positioned(
      ...
      child: TweenAnimationBuilder<double>(
        ...
        child: GestureDetector(
          ...
          child: _LevelNode(level: level, size: nodeSize, glowAnim: _glowAnim),
        ),
      ),
    );
  }
